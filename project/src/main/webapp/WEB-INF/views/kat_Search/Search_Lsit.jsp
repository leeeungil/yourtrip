...
<div id="asideSize">
	...
	<form id="content_form"
		...
		method="post">
		<div id="content_menu">
			[지역]<input type="text" value="지역선택" class="select_area" id="choice"
				onclick="area_hide()" name="select_area" readonly> [상품]<input
				type="text" value="상품선택" class="select_menu" id="choice_1"
				onclick="menu_hide() " name="select_menu" readonly> [키워드입력]<input
				type="search" placeholder=" 키워드 입력" class="menu_keyword"
	...
	<c:if test="${Searchlist.getShopInfoList().size() != 0}">
		<table class="main_table">
			<tr>
				<td><c:forEach var="search"
						items="${Searchlist.getShopInfoList()}" varStatus="status">
						<ul id="photolist">
							<li id="photoinfo"><span id="namefont">${status.count}.
									${search.business_name}</span><br> <br> <span id="infoFont"><span
									id="shopfontsize"><img class="shoppin"
										src="<%=request.getContextPath()%>/uploadfile/icon/pin.png"></span>
									위치 : ${search.address1}</span><br> <br> <span
								id="infoFont"><span id="shopfontsize"><img
										class="shopicon"
										src="<%=request.getContextPath()%>/uploadfile/icon/icon.png"></span>
									상품 분류 : <c:if test="${search.menu_catagory == null}">
									[상품 예약하기] 누르시면 상품의 정보를 볼 수 있습니다.
									</c:if> <c:if test="${search.menu_catagory != null}">
									${search.menu_catagory}
									</c:if> </span><br> <br>
								<ul id="miniMenu">
									<li><a
										href="<%=request.getContextPath()%>/kat/seatInfoLsit/reservation.do?SearchName=${search.user_id}">
											예약하기</a></li>
								</ul> <br></li>
						</ul>
					</c:forEach></td>
					...
					<ul id="photolist">
						<li id="photoinfo"><span id="namefont"> 검색한 결과가 없습니다.</span><br>
							<br> <span id="infoFont"><span id="shopfontsize"><img
									class="shoppin"
									src="<%=request.getContextPath()%>/uploadfile/icon/pin.png"></span>
								위치 : 검색한 결과가 없습니다.</span><br> <br> <span id="infoFont"><span
								id="shopfontsize"><img class="shopicon"
									src="<%=request.getContextPath()%>/uploadfile/icon/icon.png"></span>
								상품 분류 : 검색한 결과가 없습니다.</span><br> <br>
							<ul id="miniMenu">
								<li><a
